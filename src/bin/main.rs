...
enum Commands {
    ...
    /// Clone from existing sandbox storage (uses VMGS boot state)
    Clone {
        /// Sandbox name (must be unique)
        #[arg(short, long)]
        name: String,
        ...
        #[arg(short, long)]
        storage: String,
        /// Force fresh start (delete old storage and skip VMGS)
        #[arg(long)]
        fresh: bool,
    },
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Some(Commands::Clone { name, storage, fresh }) => {
            cmd_clone(&name, &storage, fresh)?;
        }
...
fn cmd_clone(name: &str, storage_id: &str, _fresh: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vhdx_path = sandbox_vhdx.to_string_lossy().to_string();
    println!("Using VHDX directly: {}", vhdx_path);

    // Build the working config (same as test that succeeded)
    let pipe_name = format!(r"\\.\pipe\hcs-sandbox-{}", name);
